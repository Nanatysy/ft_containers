...
namespace ft
{
	...
	private:
		...
		public:
			...
			_set_iterator() : _ptr(nullptr), _tree(nullptr) {}
			explicit _set_iterator(Iter data, const set<T> *x) : _ptr(data), _tree(x) {}
			_set_iterator(const _set_iterator & src)
			{
				*this = src;
			}

			_set_iterator &operator=(const _set_iterator & src)
			{
				if (this == &src)
					return (*this);
			...
			bool operator==(const _set_iterator & src)
			{
			...
			_set_iterator<Iter> operator++(int)
			{
				_set_iterator<Iter> tmp(*this);
		...
		public:
			...
			_set_reverse_iterator() : _ptr(nullptr), _tree(nullptr) {}
			explicit _set_reverse_iterator (iterator_type it, const set<T> *x) : _ptr(it), _tree(x) {}
			_set_reverse_iterator (const _set_reverse_iterator<Iter>& rev_it) {
				*this = rev_it;
			}

			_set_reverse_iterator &operator=(const _set_reverse_iterator & src)
			{
				if (this == &src)
					return (*this);
			...
			bool operator==(const _set_reverse_iterator & src)
			{
				return (this->_ptr == src._ptr);
			}
			bool operator!=(const _set_reverse_iterator & src)
			...
			_set_reverse_iterator<Iter> operator++()
			{
				this->ptr--;
				return(this);
			}
			_set_reverse_iterator<Iter> operator++(int)
			{
				_set_reverse_iterator<Iter> tmp(*this);
	...
	public:
		...
		typedef _set_iterator<t_node *> iterator;
		typedef _set_iterator<const t_node *> const_iterator;
		typedef _set_reverse_iterator<iterator> reverse_iterator;
...
//		Element access
		...
		{
			pair<iterator,bool> insert_result = insert(k);
...
//		modifiers
		pair<iterator,bool> insert (const value_type& val)
		{
			...
			pos = _find_position_for_new_node(new_elem->val);
			parent = pos.first;
			direction = pos.second;
			if (!direction && _compare(pos.first->val, new_elem->val) == false)
			{
		...
		void insert (InputIterator first, typename ft::enable_if<ft::is_iterator <InputIterator>::value, InputIterator>::type last)
		{
			...
			while (first != last)
			{
				...
				parent = pos.first;
				direction = pos.second;
				if (direction == false && _compare(pos.first->val, new_elem->val) == false)
				{
					_delete_tree(new_elem);
					first++;
		...
		size_type erase (const key_type& k)
		{
			...
			if (pos.second == true || (pos.second == false && pos.first->val != k)) // key doesn't exist
				return (0);
...
//		operations
		iterator find (const key_type& k)
		{
			...
			while (!_is_end(*tmp))
			{
				if (_compare(k, tmp->val))
					tmp = tmp->left;
				else
				{
					if (!_compare(tmp->val, k))
...
//		allocator
	...
	private:
		...
		t_node *_new_node(const value_type & value)
		{
			t_node *next = new t_node;
		...
		t_node *_new_node(const t_node & src, const set<T>& ref)
		{
			t_node *next = new t_node;
		...
		void	_child_copy(t_node **next, const t_node & src, const set<T> & ref)
		{
		...
		ft::pair<t_node *, bool> _find_position_for_new_node(const key_type &val)
		{
			...
			tmp_r = parent->right;
			tmp_l = parent->left;
			direction = _compare(val, parent->val);
			if (direction == _compare(parent->val, val)) // key already exist - key
			...
			while ((!_is_end(*tmp_l) || !direction) && (!_is_end(*tmp_r) || direction))
			{
				...
				tmp_r = parent->right;
				direction = _compare(val, parent->val);
				if (direction == _compare(parent->val, val))
